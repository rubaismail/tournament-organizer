...
// used array permutations to generate seating arrangements, stops when it finds the first valid permutation and prints it
void printFirstPerm(int *perm, int *used, int k, int n, int* foundValidPerm) {
    ...
    if (k == n)
    {
        // checks if the person to the left or right has snacks
    ...
    for (int i = 0; i < n; i++)
    {
        if (used[i] == 0)
        {
            ...
            perm[k] = i;
            used[i] = 1;
            printFirstPerm(perm, used, k + 1, n, foundValidPerm);
            used[i] = 0;
...
int main(void)
{
    ...
    int foundSolution = 0;
    printFirstPerm(perm, used, 0, n, &foundSolution);
